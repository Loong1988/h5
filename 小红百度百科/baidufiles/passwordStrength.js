    ...
    a.fn.passwordStrength = function(b) {
        ...
        this.each(function() {
            ...
            function() {
                ...
                e >= 0 && c == false && (c = true);
                f.find(".passwordStrength div").removeClass("active");
                if (e < 35 && e >= 0) {
                    f.find(".pass_strength02").addClass("active").siblings().removeClass("active");
                } else {
                    if (e < 60 && e >= 35) {
                        f.find(".pass_strength03").addClass("active").siblings().removeClass("active");
                    } else {
                        if (e >= 60) {
                            f.find(".pass_strength04").addClass("active").siblings().removeClass("active");
                        }else{
							f.find(".pass_strength01").addClass("active").siblings().removeClass("active");
						}
                    }
                }
            });
            d.bind("blur",
                    function() {
                        ...
                        e >= 0 && c == false && (c = true);
                        if (d.val().length == 0) {
                            b.showmsg(d, d.attr("nullmsg"), 3)
        	            } else if (c && (d.val().length < b.minLen || d.val().length > b.maxLen)) {

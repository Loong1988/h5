(function(a) {
    a.fn.olvSlides = function(b) {
        a.fn.olvSlides.option = {container: "container",control: "control",change: "change",createNextPrev: true,next: "next",prev: "prev",createPagination: true,paginationClass: "pageClass",currentClass: "cur",thumb: false,thumbPage: true,thumbPrev: "thumbPrev",thumbNext: "thumbNext",thumbDirection: "X",bigTarget: false,bigTargetPrev: "bigTargetPrev",bigTargetNext: "bigTargetNext",fadeSpeed: 350,slideSpeed: 350,start: 1,effect: "slide",fadeEasing: "",play: 5000,pause: 2000,hoverPause: false};
        ...
        return this.each(function() {
                ...
                if (!m) {
                    ...
                    if (J === "slide") {
                        ...
                        q.children(":eq(" + v + ")", B).css({zIndex: 8}).fadeIn(b.fadeSpeed, b.fadeEasing, function() {
							...
							else if(H=1){q.children(":eq(" + u + ")", B).css({display: "block",zIndex: 0});}
            ...
            if (b.thumb) {
                ...
                if (b.thumbDirection === "Y") {
					...
					if(H<4){s=0;}else{s = -p * H + D;}
                    F = C.children(":eq(0)").position().top;
                    C.children(":eq(0)").css({height: p * H + "px",width: "100%",position: "absolute",left: "0px",top: "0px"});
                    if (b.start > f) {
                    ...
                    s = -p * H + D;
                    F = C.children(":eq(0)").position().left;
                    C.children(":eq(0)").css({width: p * H + "px",height: "100%",position: "absolute",left: "0px",top: "0px"});
                    if (b.start > f) {
                        if ("-" + Math.ceil((p * startT) - (D / 2) - (p / 2)) < -p * H + D) {
                ...
                C.children(":eq(0)").children().each(function(L) {
                    ...
                    var K = '<li><div class="imgWrap"><a href="' + J + '" data-title="' + I + '" class="example-image-link"  data-lightbox="example-set"><img src="' + N + '" alt="' + I + '" /></a></div><div class="opacity"></div><div class="textDesc"><div class="title"><a href="' + J + '" target="_blank">' + I + '</a></div><div class="text">' + M + "</div></div></li>";
                    a(this).data("data", L);
                    q.append(K)
                });
